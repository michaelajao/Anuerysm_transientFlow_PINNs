...
from torch.cuda.amp import autocast, GradScaler  # Updated import
...
def setup_logging(run_id: str, metrics_dir: str, config: Config) -> logging.Logger:
    ...
    if not logger.handlers:
        ...
        os.makedirs(os.path.join(metrics_dir, run_id), exist_ok=True)

        # Create file handler
        file_handler = logging.FileHandler(os.path.join(metrics_dir, run_id, f"experiment_{run_id}.log"), mode="a")
...
def compute_inlet_loss(
    u_pred: torch.Tensor, v_pred: torch.Tensor, w_pred: torch.Tensor, t: torch.Tensor, heart_rate: int = 120
) -> torch.Tensor:
    """
...
def train_pinn(
    ...
    """
    scaler = GradScaler()  # Updated usage
...
def plot_pressure_and_wss_magnitude_distribution(
    ...
    def plot_scatter(ax, x, y, c, title, xlabel, ylabel, cmap, label, vmin=None, vmax=None, num_ticks=5):
        ...
        # Create colorbar with limited ticks
        cbar = plt.colorbar(sc, ax=ax, label=label, ticks=np.linspace(vmin, vmax, num_ticks))
        cbar.ax.tick_params(labelsize=10)
        
        # Optional: Format tick labels (e.g., reduce decimal places)
        cbar.ax.yaxis.set_major_formatter(plt.FormatStrFormatter('%.2f'))

    # 1. Pressure Distribution in XY and XZ planes
    ...
    plot_scatter(
        ...
        vmin=pressure_min,
        vmax=pressure_max,
        num_ticks=5  # Limit number of ticks
    )
    # PINN Pressure (XY Plane)
    plot_scatter(
        axs_p[0, 1],
        x_sample,
        y_sample,
    ...
    # 2. WSS Magnitude Distribution in XY and XZ planes
    fig_wss, axs_wss = plt.subplots(2, 2, figsize=(12, 8))
    ...
    # CFD WSS Magnitude (XY Plane)
    plot_scatter(
        axs_wss[0, 0],
        x_sample,
        y_sample,
        wss_magnitude_true,
        "CFD WSS Magnitude (XY)",
        "X [m]",
        "Y [m]",
        "inferno",
        "WSS [Pa]",
    ...
    # PINN WSS Magnitude (XY Plane)
    plot_scatter(
        axs_wss[0, 1],
        x_sample,
        y_sample,
        wss_magnitude_pred,
        "PINN WSS Magnitude (XY)",
        "X [m]",
        "Y [m]",
        "inferno",
        "WSS [Pa]",
    ...
    # CFD WSS Magnitude (XZ Plane)
    plot_scatter(
        axs_wss[1, 0],
        x_sample,
        z_sample,
        wss_magnitude_true,
        "CFD WSS Magnitude (XZ)",
        "X [m]",
        "Z [m]",
        "inferno",
        "WSS [Pa]",
    ...
    # PINN WSS Magnitude (XZ Plane)
    plot_scatter(
        axs_wss[1, 1],
        x_sample,
        z_sample,
        wss_magnitude_pred,
        "PINN WSS Magnitude (XZ)",
        "X [m]",
        "Z [m]",
        "inferno",
        "WSS [Pa]",
    ...
    plt.tight_layout(rect=[0, 0.03, 1, 0.95])
    fig_wss.suptitle(f"WSS Magnitude Distribution - {run_id}", fontsize=16)
